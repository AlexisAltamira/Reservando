...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    var restaurantElegido = this.restaurantes.filter(restaurant => restaurant.id == id);

    if (restaurantElegido.length > 0) {
        return restaurantElegido[0];
    }
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var horarios = this.restaurantes.map(function(restaurant){
        return restaurant.horarios;
    });
    
    //En este arreglo vamos a poner todos los horarios, uno por uno
    const arregloHorarios = [];
    horarios.forEach(function(elem) {
        elem.forEach(function(hor) {
            arregloHorarios.push(hor);
        });
    });
    return this.eliminarRepetidos(arregloHorarios).sort();
